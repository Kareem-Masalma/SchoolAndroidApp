...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    if (isset($_GET["id"])) {
        $id = $_GET["id"];
        $sql = "SELECT * FROM teacher t, user u WHERE u.id = ? AND t.teacher_id = u.id";
        ...
        $sql = "SELECT * FROM teacher t, user u WHERE t.teacher_id = u.id";
    ...
    if ($user_stmt->execute()) {
        ...
        if ($teacher_stmt->execute()) {
            ...
            echo json_encode(["success" => false, "error" => "Failed to insert into teacher table"]);
        }
    ...
    if ($user_stmt->execute()) {
        if (!empty($speciality)) {
            ...
            if ($teacher_stmt->execute()) {
                ...
                echo json_encode(["success" => false, "error" => "Failed to update teacher table"]);
            }
            $teacher_stmt->close();
        } else {
            echo json_encode(["success" => true, "message" => "Teacher updated successfully"]);
