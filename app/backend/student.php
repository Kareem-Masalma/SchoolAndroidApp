...
if ($_SERVER["REQUEST_METHOD"] == "GET") {
    ...
    if ($user_stmt->execute()) {
        ...
        $student_stmt = $conn->prepare("INSERT INTO students (student_id, class_id) VALUES (?, ?)");
        $student_stmt->bind_param("ii", $user_id, $class_id);

        if ($student_stmt->execute()) {
            echo json_encode(["success" => true, "message" => "Student added successfully"]);
        } else {
            echo json_encode(["success" => false, "error" => "Failed to insert into students table"]);
        }
    ...
    if ($user_stmt->execute()) {
        if ($class_id) {
            $student_stmt = $conn->prepare("UPDATE students SET class_id=? WHERE student_id=?");
    ...
    $stmt1 = $conn->prepare("DELETE FROM students WHERE student_id=?");
